...
public class _2PSwipeMenu : MonoBehaviour
{
    ...
    {
        touch = false;
        //scrollBar.GetComponent<Scrollbar>().value = Mathf.Lerp(scrollBar.GetComponent<Scrollbar>().value, pos[1], 0.02f);
    }
    private void FixedUpdate()
    {
        ...
        for (int i = 0; i < pos.Length; i++)
        {
            ...
            if (scroll_pos < pos[i] + (scrollDistance / 2) && scroll_pos > pos[i] - (scrollDistance / 2))
            {
                ...
                for (int a = 0; a < pos.Length; a++) //Eğer butonumun indeksi i'ye eşit deilse küçült
                {
                    if (a != i)
                    {
